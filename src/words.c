 ...
 */
	...
	else if(!strcmp("spaces", word)){ ltemp[0] = pop(0); if(!get_error()) for(i=0; (long)i < ltemp[0]; i++) putchar(' ');}
	...
	else if(!strcmp("type", word)){ ltemp[0] = pop(0); stemp = (char*)pop(0); for(i=0; (long)i < ltemp[0]; i++)putchar(stemp[i]); pop_string(stemp);} //TODO check for segfaults?
	else if(!strcmp(".", word)){ ltemp[0] = pop(0); if(!get_error()) printf("%ld", ltemp[0]); fflush(stdout);}
	...
	else if(!strcmp("<>", word)){ ltemp[0] = pop(0); if((pop(0) ^ ltemp[0]) != 0) push(_TRUE); else push(_FALSE);} //XOR's to check bit-for-bit not equal to
	...
	else if(!strcmp("xor", word)){ ltemp[0] = pop(0); push(ltemp[0] ^ pop(0)); }
	else if(!strcmp("or", word)){ ltemp[0] = pop(0); push(ltemp[0] | pop(0)); }
	else if(!strcmp("and", word)){ ltemp[0] = pop(0); push(ltemp[0] & pop(0)); }
	else if(!strcmp("swap", word)){ ltemp[0] = pop(0); ltemp[2] = pop(0); push(ltemp[0]); push(ltemp[2]); }
	else if(!strcmp("2swap", word)){ for(i = 0; i < 4; i++) ltemp[i] = pop(0); push(ltemp[1]); push(ltemp[0]); push(ltemp[3]); push(ltemp[2]); }
